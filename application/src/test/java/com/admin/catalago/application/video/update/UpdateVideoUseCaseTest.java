...
import com.admin.catalago.domain.exceptions.InternalErrorException;
...
public class UpdateVideoUseCaseTest extends UseCaseTest {
    ...
    public void givenAValidCommand_whenCallsUpdateVideo_shouldReturnVideoId() {
        ...
        final Resource expectedThumb = Fixture.Videos.resource(VideoMediaType.THUMB);
    ...
    private void mockImageMedia() {
        when(mediaResourceGateway.storeImage(any(), any())).thenAnswer(t -> {
            ...
            return ImageMedia.with(resource.resource().checksum(), name, "/raw/".concat(name));
        });
    }
    ...
    private void mockAudioVideoMedia() {
        when(mediaResourceGateway.storeAudioVideo(any(), any())).thenAnswer(t -> {
            final var resource = t.getArgument(1, VideoResource.class);
            final var name = resource.resource().name();
            return AudioVideoMedia.with(resource.resource().checksum(), name, "/raw/".concat(name));
        });
    }
}